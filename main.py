...
def calcular_beneficio(contribuicoes, salario_min=1412.00):
    ...
    valores = [contribuicoes[data] for data in datas]
    media = sum(valores) / len(valores)
    return max(media, salario_min)
...
def verificar_elegibilidade(dados):
    ...
    elegibilidade["qualidade_segurada"] = True
    elegibilidade["dentro_prazo"] = True
    if not pagou_12:
...
def verificar_informacoes_incompletas(dados):
    faltantes = []
    if not dados.get("rg"):
        faltantes.append("RG da Mãe")
    if not dados.get("certidao"):
        faltantes.append("Certidão de Nascimento do Filho")
    if not dados.get("cnis"):
        faltantes.append("Extrato/CNIS")
    if not dados.get("pis"):
        faltantes.append("PIS/PASEP")
    if dados.get("ocupacao") == "MEI" and not dados.get("mei"):
        faltantes.append("Comprovante MEI")
    return faltantes

# ========================================
# Função para Calcular Aprovação (Escala)
# ========================================
def calcular_aprovacao(dados, ocupacao):
    ...
    if dados.get("cpf_valido"):
        score += 1
    if not verificar_informacoes_incompletas(dados):
        score += 1
    if ocupacao == "CLT":
        score += 1
...
def renomear_arquivo(caminho_arquivo, cpf, nome, tipo_doc):
    ...
    nome_limpo = " ".join(nome.split()).replace(" ", "")
    if nome_limpo.endswith("CPF"):
        nome_limpo = nome_limpo[:-3]
    novo_nome = f"{cpf_limpo}_{nome_limpo}_{tipo_doc}{extensao}"
...
# ========================================
# Geração do Relatório Final (Organizado e Compacto)
# ========================================
def gerar_relatorio(dados):
    relatorio = {
        ...
        "informacoes_incompletas": verificar_informacoes_incompletas(dados)
    }
    return relatorio

# ========================================
# Processamento dos Documentos (Segmentado)
# ========================================
def processar_documentos(form_files, form_data):
    ...
    for campo in ['rg', 'certidao', 'cnis', 'pis', 'mei']:
        ...
        if file:
            ...
            if dados_aggregados.get("cpf") and dados_aggregados.get("nome"):
                ...
                info_doc["arquivo_renomeado"] = novo_caminho
    ...
    dados_aggregados["elegibilidade"] = verificar_elegibilidade(dados_aggregados)
...
def index():
    ...
    <body>
      ...
      <div class="content">
        ...
        <p>Este é o VAII, um sistema desenvolvido pela Mútua - Assessoria Maternidade para automatizar a análise de documentos destinados à solicitação de benefícios do INSS.</p>
        ...
        <p>Além disso, o sistema verifica se todos os documentos obrigatórios estão presentes e alerta se algum estiver faltando ou se a qualidade do arquivo for baixa.</p>
        <a class="button" href="/upload_form">Fazer Upload dos Documentos</a>
      </div>
      <div class="footer">
        &copy; 2025 Mútua - Assessoria Maternidade. Todos os direitos reservados.
      </div>
    </body>
    </html>
    """
    ...
    <body>
      <div class="upload-container">
        ...
        <form id="formUpload" method="POST" action="/upload_form" enctype="multipart/form-data" onsubmit="enviarFormulario(event)">
          ...
          <input type="file" name="rg" accept=".pdf,.jpg,.jpeg,.png" required>
          ...
          <input type="file" name="cnis" accept=".pdf,.jpg,.jpeg,.png" required>

          <label for="pis">PIS/PASEP:</label>
          <input type="file" name="pis" accept=".pdf,.jpg,.jpeg,.png" required>
          ...
          <div id="campo_mei" style="display:none;">
            ...
            <input type="file" name="mei" accept=".pdf,.jpg,.jpeg,.png">
          </div>
...
# ========================================
# Interface Web: Relatório e Opção de Baixar PDF
# ========================================
...
def upload_form_post():
    ...
    relatorio = processar_documentos(form_files, form_data)
    aprovacao = calcular_aprovacao(relatorio, ocupacao)
    relatorio["aprovacao"] = aprovacao
    ...
    <body>
      <div class="container">
          ...
          {% for doc in dados.documentos_processados %}
            <div class="data">
              ...
              {% if doc.qualidade.startswith("OK") and doc.get("arquivo_renomeado") %}
                <a class="button" href="/download/{{ doc.arquivo_renomeado.split('/')[-1] }}">Baixar</a>
              {% endif %}
            </div>
        ...
        <div style="text-align:center; margin-top:10px;">
          ...
          <a class="button" href="/download_report_pdf?report={{ report_filename }}">Baixar Relatório em PDF</a>
        </div>
      </div>
    </body>
    </html>
    """, dados=relatorio, report_filename=f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html")
...
# ========================================
# Rota para Gerar e Baixar o PDF do Relatório
# ========================================
@app.route("/download_report_pdf")
def download_report_pdf():
    report_filename = request.args.get("report")
    if not report_filename:
        return "Relatório não encontrado.", 400
    report_path = os.path.join("reports", report_filename)
    if not os.path.exists(report_path):
        return "Relatório não encontrado.", 404
    with open(report_path, "r", encoding="utf-8") as f:
        html_content = f.read()
    pdf = gerar_pdf_report(html_content)
    if pdf is None:
        return "Erro ao gerar PDF", 500
    # Incluir o nome do cliente no nome do arquivo, se disponível
    client_name = request.form.get("nome", "").replace(" ", "_") if request.form.get("nome") else "cliente"
    filename = f"{client_name}_relatorio_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    return send_file(pdf, as_attachment=True, download_name=filename, mimetype='application/pdf')

# ========================================
# Rota para Servir openapi.yaml
# ========================================
...
# ========================================
# Rota para Servir ai-plugin.json
# ========================================
